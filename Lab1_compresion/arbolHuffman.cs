...
namespace Lab1_compresion
{
    public class arbolHuffman
    {
        ...
        public nodoH cargar(List<string> arbol)
        {
            nodoH root = null;
            if (arbol.Count != 0)
            {
                ...
                {
                    root = new nodoH();
                    root.simbolo = arbol[0][1];
                    root.Frecuencias = int.Parse(arbol[0].Substring(2));
                    arbol.RemoveAt(0);
                }
                else
                {
                    ...
                    root.Frecuencias = int.Parse(arbol[0].Substring(2));
                    arbol.RemoveAt(0);
                    root.izquierda = cargar(arbol);
                    root.derecha = cargar(arbol);
                }
            }
            return root;
        }
